...
async def force_import():
    ...
    user_email = "labhomeopatico@chekadmin.com"
    db_name = "checkadmin_prod"
    file_path = r"C:\Users\keine\Downloads\homeopatia  plantilla_inventario.xlsx"
    ...
    # 3. Get warehouses for this company
    ...
    print(f"Mapping warehouses: {wh_name_map}")
    ...
    for index, row in df.iterrows():
        try:
            ...
            if not sku or name == 'nan' or not name:
                continue
                
            wh_name_input = str(get_col_value(row, col_map['bodega'], '')).strip().lower()
            warehouse_id = wh_name_map.get(wh_name_input)
